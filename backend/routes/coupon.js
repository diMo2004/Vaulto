 ...
 */
// GET /coupons/all – get all coupons for the logged-in user
router.get("/all", authMiddleware, async (req, res) => {
  try {
    const now = new Date();

    // 1) Delete all expired coupons for this user
    await Coupon.deleteMany({
      owner: req.user.id,
      expiry: { $lt: now },
    });

    // 2) Return remaining coupons (sorted newest first)
    const coupons = await Coupon.find({ owner: req.user.id })
      .sort({ createdAt: -1 })
      .lean();
...
/**
 * GET /coupons/expiring-soon?days=3
 * Returns coupons owned by the user that expire within the next N days
 */
router.get("/expiring-soon", authMiddleware, async (req, res) => {
  try {
    const days = parseInt(req.query.days || "3", 10); // "near future" = 3 days by default
...
// DELETE /coupons/:id – delete one coupon by id (only if it belongs to the user)
