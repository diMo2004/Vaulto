...
- `SPRING_DATA_MONGODB_URI`, `MONGO_URI`, or `MONGODB_URI`
- `JWT_ACCESS_SECRET`
- `JWT_REFRESH_SECRET`
- `GOOGLE_CLIENT_ID`
...
| Google callback fails with MongoDB `localhost:27017` connection refused | Railway backend does not see the production MongoDB URI, or the backend was not redeployed after variables changed | Set `SPRING_DATA_MONGODB_URI` or `MONGO_URI` on the Railway backend service, then redeploy the backend. |
